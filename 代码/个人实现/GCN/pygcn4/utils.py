...
def my_load_data(dataset, func):
    ...
    # print(tmp_adj)
    graph = [list() for i in range(len(tmp_adj))]
    ...
    for i, line in enumerate(tmp_adj):
        for j, paper in enumerate(line):
            paper1 = i
            paper2 = j
            if int(tmp_adj[i][j]) > 0:
                graph[paper1].append(paper2)
                graph[paper2].append(paper1)
    # for i in range(len(graph)):
    #     print(i,":",graph[i])
    #观察最大的neighbourhood
    mean_len = 0
    for i in range(len(graph)):
        mean_len = mean_len + len(graph[i])
    mean_len = mean_len / len(graph)
    print("mean_neighbour = ", mean_len)
...
def load_flickr(dataset_source, func):
    data = sio.loadmat("../data/{}.mat".format(dataset_source))
    features = data["Attributes"]
    adj = data["Network"]
    labels = data["Label"]
...
def load_youtube(dataset_source, func):
    #内存不够跑不了
    data = sio.loadmat("../data/{}.mat".format(dataset_source))
    #features = data["Attributes"]
    adj = data["network"]
    labels = data["group"]
    tmp_adj = np.array(adj.todense())
    features = [i for i in range(len(tmp_adj))]
    #print(adj)
    nb_nodes = features.shape[0]
    ft_size = features.shape[1]
